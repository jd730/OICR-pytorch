...
def clip_gradient(model, clip_norm):
    ...
    totalnorm = 0
    for pw in model.parameters():
        if pw.requires_grad: 
            try :
                modulenorm = pw.grad.data.norm()
            except :
                pdb.set_trace()
            modulenorm = modulenorm.cuda(0)
...
def vis_gts(im, img_path) :
    """ Visualize GT"""
    anno_path = img_path.replace('JPEGImages','Annotations').replace('jpg','xml')
    ...
    for ix, obj in enumerate(objs):
        ...
        bnb = obj.find('bndbox')
        x1 = int(bnb.find('xmin').text)
        y1 = int(bnb.find('ymin').text)
        x2 = int(bnb.find('xmax').text)
        y2 = int(bnb.find('ymax').text)
        cv2.rectangle(im, (x1,y1), (x2,y2), (0,0,204),2)
        cv2.putText(im, cls, (x1, y1+15), cv2.FONT_HERSHEY_PLAIN, 1.0,(0,0,255), thickness=1)
    return im
